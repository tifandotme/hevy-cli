...
export async function fetchAllPages<T extends Record<string, unknown>>(options: {
  ...
  const remainingResults = await Promise.all(remainingPages.map((page) => options.fetchPage(page)));
