...
export async function readLocalAuthConfig(options: AuthOptions = {}): Promise<AuthConfig> {
...
export async function resolveApiKey(options: AuthOptions = {}): Promise<string> {
  const env = options.env ?? Bun.env;
  if (env.HEVY_API_KEY) {
  ...
  throw new Error("Missing Hevy API key. Set HEVY_API_KEY or run `hevy auth login`.");
}

export async function saveApiKey(apiKey: string, options: AuthOptions = {}): Promise<string> {
