...
test("readJsonBody parses stdin JSON", async () => {
  await expect(readJsonBody("-", createMemoryStdio('{"a":1}'))).resolves.toEqual({ a: 1 });
});

test("readJsonBody reports invalid JSON", async () => {
  await expect(readJsonBody("not json", createMemoryStdio())).rejects.toThrow("Invalid JSON body");
});
